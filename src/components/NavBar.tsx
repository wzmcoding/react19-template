...
const NavBar: React.FC = () => {
    ...
    return (
        <nav className="h-screen w-62 flex flex-col border-r border-secondary-foreground/9 border-solid px-3.5 pt-7">
            ...
            {nav.map((item, index) => (
                <Link
                    key={index}
                    to={item.value}
                    className={`my-3 flex cursor-pointer items-center gap-1 rounded-xl p-2 ${
                        isActive(item.value)
